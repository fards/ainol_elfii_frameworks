...
public:
    ...
    status_t set3DFormat(const sp<SurfaceComposerClient>& client, SurfaceID id,
            uint32_t format);	
...
status_t Composer::set3DFormat(const sp<SurfaceComposerClient>& client,
        SurfaceID id, uint32_t format ) {
    LOGI("Composer::set3DFormat");    
    Mutex::Autolock _l(mLock);
    layer_state_t* s = getLayerStateLocked(client, id);
    if (!s)
        return BAD_INDEX;

    s->what |= ISurfaceComposer::e3DFormatChanged;
    s->a3dformat= format;
    return NO_ERROR;
}
...
status_t SurfaceComposerClient::set3DFormat(SurfaceID id, uint32_t format)
{
    return getComposer().set3DFormat(this, id, format);
}
