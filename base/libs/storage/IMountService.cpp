...
public:
    ...
    virtual bool isUsbMassStorageAllEnabled()
    {
        ...
        if (remote()->transact(TRANSACTION_isUsbMassStorageAllEnabled, data, &reply) != NO_ERROR) {
            LOGD("isUsbMassStorageAllEnabled could not contact remote\n");
            return false;
        }
        int32_t err = reply.readExceptionCode();
        if (err < 0) {
            LOGD("isUsbMassStorageAllEnabled caught exception %d\n", err);
            return false;
        }
        return reply.readInt32() != 0;
    }
    ...
    int32_t getVolumeLabel(const String16& mountPoint)
    {
        ...
        if (remote()->transact(TRANSACTION_getVolumeLabel, data, &reply) != NO_ERROR) {
            LOGD("getVolumeLabel could not contact remote\n");
            return -1;
        }
        int32_t err = reply.readExceptionCode();
        if (err < 0) {
            LOGD("getVolumeLabel caught exception %d\n", err);
            return err;
        }
        return reply.readInt32();
    }
    ...
    int32_t getVolumeFSLabel(const String16& mountPoint)
    {
        ...
        if (remote()->transact(TRANSACTION_getVolumeFSLabel, data, &reply) != NO_ERROR) {
            LOGD("getVolumeFSLabel could not contact remote\n");
            return -1;
        }
        int32_t err = reply.readExceptionCode();
        if (err < 0) {
            LOGD("getVolumeFSLabel caught exception %d\n", err);
            return err;
        }
        return reply.readInt32();
    }
