...
status_t EventHub::openDeviceLocked(const char *devicePath) {
    ...
    if (test_bit(BTN_MOUSE, device->keyBitmask)
        ...
        if(//!test_bit(BTN_FORWARD, device->keyBitmask)
            //&& !test_bit(BTN_BACK, device->keyBitmask)
            //&& !test_bit(BTN_SIDE, device->keyBitmask)
            //&& !test_bit(BTN_EXTRA, device->keyBitmask)
            test_bit(BTN_LEFT, device->keyBitmask)
            && test_bit(BTN_RIGHT, device->keyBitmask)){
            device->classes |= INPUT_DEVICE_CLASS_STANDARD_MOUSE;
        }
    }
