...
void InputReader::updateInputConfigurationLocked() {
    int32_t touchScreenConfig = InputConfiguration::TOUCHSCREEN_NOTOUCH;
        ...
        if(property_get("ro.platform.has.touch", value, NULL) > 0 ){
            if (!strcmp(value, "0") || !strcmp(value, "false")){//for support third apk that has finger resource when no touch
...
uint32_t CursorButtonAccumulator::getButtonState( InputDevice* device ) const {
    uint32_t result = 0;
    if (mBtnLeft) {
        result |= AMOTION_EVENT_BUTTON_PRIMARY;
    }
    if(device->getClasses() & INPUT_DEVICE_CLASS_STANDARD_MOUSE){
        if (mBtnRight) {
...
void TouchButtonAccumulator::reset(InputDevice* device) {
    ...
    mBtnHome = device->isKeyPressed(KEY_HOME);
...
bool TouchButtonAccumulator::isToolActive() const {
    return mBtnTouch || mBtnHome || mBtnToolFinger || mBtnToolPen || mBtnToolRubber
...
void CursorInputMapper::sync(nsecs_t when) {
    ...
    int32_t currentButtonState = mCursorButtonAccumulator.getButtonState(getDevice());
...
TouchInputMapper::TouchInputMapper(InputDevice* device) :
        InputMapper(device),
        mSource(0), mTvOutStatus(false), mPadmouseStatus(false),
...
void TouchInputMapper::sync(nsecs_t when) {
    ...
    mCurrentButtonState = mTouchButtonAccumulator.getButtonState()
            | mCursorButtonAccumulator.getButtonState(getDevice());
