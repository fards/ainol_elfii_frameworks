...
class MountService extends IMountService.Stub
     ...
     * Only updating PackageManager when changes happen on this storage path
     */
    private static final String EXTERNAL_STORAGE2_PATH = Environment.getExternalStorage2Directory().toString();
    ...
    private static final int H_UNMOUNT_PM_SHUTDOWN = 4;
    private static final int RETRY_UNMOUNT_DELAY = 30; // in ms
    private static final int MAX_UNMOUNT_RETRIES = 4;

    private static final int SHUTDOWN_RETRYIES_NUM = 30;
    private int mShutdown_retries = SHUTDOWN_RETRYIES_NUM;
    private boolean mShutdowning = false;
    private static final int SHUTDOWN_UNMOUNT_STAGE_INIT = 0;
    private static final int SHUTDOWN_UNMOUNT_STAGE_EXTRA = 1;
    private int mShutdownUnmountStage = SHUTDOWN_UNMOUNT_STAGE_INIT;
    ...
    class MountServiceHandler extends Handler {
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                case H_UNMOUNT_PM_SHUTDOWN:
                    IMountShutdownObserver observer = (IMountShutdownObserver) msg.obj;
                    shutdown(observer);
                    break;
                case H_UNMOUNT_PM_UPDATE: {
                    ...
                    if (ucb.path == null || !mUpdatingStatus.contains(ucb.path)) {
                        ...
                        if (ucb.path != null && (ucb.path.equals(EXTERNAL_STORAGE_PATH) || ucb.path.equals(EXTERNAL_STORAGE2_PATH)))
                       	{
                       	    if(ucb.path.equals(EXTERNAL_STORAGE_PATH)){
                       	    	    if(mUpdatingStatus.contains(EXTERNAL_STORAGE2_PATH))
					 break;
          	    	        }
	            ...
	            if(donePath.equals(EXTERNAL_STORAGE_PATH))	{
			if(mUpdatingStatus.contains(EXTERNAL_STORAGE2_PATH)){
				donePath=EXTERNAL_STORAGE2_PATH;
				}
	            	}
                    boolean bNeedMsgRetry=false;
		    ...
		    if(donePath.equals(EXTERNAL_STORAGE2_PATH))	{
			if(mUpdatingStatus.contains(EXTERNAL_STORAGE_PATH)){
				donePath=EXTERNAL_STORAGE_PATH;
				continue;
				}
		    }
		    break;
    ...
    private int doMountVolume(String path) {
        ...
        if(mShutdowning){
            if (DEBUG_EVENTS) Slog.i(TAG, "doMountVolume: Mount " + path+" when shutdowning");
            return StorageResultCode.OperationFailedNoMedia;
        }

        if (DEBUG_EVENTS) Slog.i(TAG, "doMountVolume: Mounting " + path);
        try {
    ...
    private int doUnmountVolume(String path, boolean force, boolean removeEncryption) {
        ...
        if (path.equals(EXTERNAL_STORAGE_PATH) || path.equals(EXTERNAL_STORAGE2_PATH))
            mPms.updateExternalMediaStatus(false, false);
        try {
    ...
    public void shutdown(final IMountShutdownObserver observer) {
        ...
        Slog.i(TAG, "Shutting down, retry count("+mShutdown_retries+")");
        ...
        synchronized (mVolumeStates) {
            for (String path : mVolumeStates.keySet()) {
                ...
                if (state.equals(Environment.MEDIA_SHARED)) {
                    ...
                    if(  mShutdownUnmountStage > SHUTDOWN_UNMOUNT_STAGE_INIT || "/mnt/sdcard/external_sdcard".equals(path) ){
                        ShutdownCallBack ucb = new ShutdownCallBack(path, observer);
                        mHandler.sendMessage(mHandler.obtainMessage(H_UNMOUNT_PM_UPDATE, ucb));
                        }
                    }

            if("/mnt/sdcard/external_sdcard".equals(path) && !state.equals(Environment.MEDIA_MOUNTED)
                && !state.equals(Environment.MEDIA_CHECKING)){
                Slog.i(TAG, "/mnt/sdcard/external_sdcard umount");
                mShutdownUnmountStage = SHUTDOWN_UNMOUNT_STAGE_EXTRA;
                    }
                }
        }
        ...
        --mShutdown_retries;
                    // Post a unmount message.
        if (observer != null) {
            if((hasChecking && mShutdown_retries > 0) || hasMounted ){
                mHandler.sendMessageDelayed(mHandler.obtainMessage(H_UNMOUNT_PM_SHUTDOWN, observer), 1000);
                return;
            } 
            Slog.e(TAG, "Timed out waiting for all media to umount");
                    /*
    ...
    public String getVolumeState(String mountPoint) {
        synchronized (mVolumeStates) {
            ...
            if (state == null) {
                ...
                if (mountPoint.startsWith("/mnt/sd") ||
   ...
   /**
    ...
    public String[] getSecureContainerList() {
        ...
        String[] rdata = null;
        try {
            rdata = mConnector.doListCommand("asec list", VoldResponseCode.AsecListResult);
        } catch (NativeDaemonConnectorException e) {
        }
        if (rdata != null && rdata.length != 0) {
            Slog.d(TAG, "getSecureContainerList returning response, size=" +
                rdata.length);
            return rdata;
        }
        synchronized (mAsecMountSet) {
            if (!mAsecMountSet.isEmpty()) {
                Slog.d(TAG, "getSecureContainerList returning mAsecMountSet, size=" +
                    mAsecMountSet.size());
                return mAsecMountSet.toArray(new String[0]);
            }
            if (mPrevAsecMountSet != null) {
                Slog.d(TAG, "getSecureContainerList returning mPrevAsecMountSet, size=" +
                    mPrevAsecMountSet.length);
                return mPrevAsecMountSet;
            }
        }
        Slog.d(TAG, "getSecureContainerList returning empty list");
        return new String[0];
    }
    ...
    public int createSecureContainer(String id, int sizeMb, String fstype,
        ...
        if (rc == StorageResultCode.OperationSucceeded) {
            synchronized (mAsecMountSet) {
                if (LOCAL_LOGD) Slog.d(TAG, "mAsecMountSet.add " + id);
        ...
        if (rc == StorageResultCode.OperationSucceeded) {
            synchronized (mAsecMountSet) {
                if (mAsecMountSet.contains(id)) {
                    if (LOCAL_LOGD) Slog.d(TAG, "mAsecMountSet.remove " + id);
