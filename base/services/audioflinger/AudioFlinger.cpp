...
bool AudioFlinger::MixerThread::threadLoop()
{
    ...
    while (!exitPending())
    {
        ...
        if (sleepTime == 0) {
            ...
            if (bytesWritten > 0) memset(mMixBuffer, 0, bytesWritten);
...
{
    if (mCblk != NULL) {
       LOGD("RecordTrack constructor %p, buffer start %p to %p end",this,(int)mBuffer, (int)mBufferEnd  );
...
AudioFlinger::RecordThread::RecordTrack::~RecordTrack()
{
    LOGD("RecordTrack descontructor %p",this);
    sp<ThreadBase> thread = mThread.promote();
    if (thread != 0) {
