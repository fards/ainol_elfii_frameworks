...
bool LayerBase::set3DFormat(uint8_t format)
{
	LOGI(   "set3DFormat %d",format );
    if (mCurrentState.a3dformat == format)
        return false;
    mCurrentState.a3dformat = format;
    requestTransaction();
    return true;
}
...
void check2XScale(char* tmp,int size )
{
    int fd;
	int subtitle_num = 0;
    char *path = "/sys/class/graphics/fb0/scale";    
...
void LayerBase::drawWithOpenGL(const Region& clip) const
{
	...
	bool need2XScale=mFlinger->is2XScale();
    char value[16];
	int offsetx=0,offsety=0;
	bool need2XScale_new=false; 
	
/*	int sizetp=property_get("rw.fb.needxyscale",value,0); //to be deleted ,don't use this.
	if(sizetp>=2)
		need2XScale_new = true;
	if(need2XScale_new)
	{
		char tmp[16];
		check2XScale(tmp,16);
		if(strncmp(tmp,"scale:[0x10001]",15)!=0)
		{
			need2XScale_new=false;
		}
	}	
	...
	if(need2XScale)
	{
		char tmp[16];
		check2XScale(tmp,16);
		if(strncmp(tmp,"scale:[0x10000]",15)!=0)
		{
			need2XScale=false;
		}
	}	
	...
	if(need2XScale)
	{
		...
		{
			Orientation=1;
			if(mOrientation & HAL_TRANSFORM_FLIP_V )
			{
				Orientation=3;
			}
		}else if(mOrientation & HAL_TRANSFORM_FLIP_V)
		{
			Orientation=2;
		}
	}
		...
		if(Orientation==0) 
   		{
			texCoords[2].u = 2;
    		texCoords[3].u = 2;
    		if(mCurrentState.x+mCurrentState.w<=fbWidth)
				offsetx=-mCurrentState.x/2;
   		}else if(Orientation==1 ) 
   		{	
   			texCoords[0].v = 2;
			texCoords[3].v = 2;
			if(mCurrentState.y+mCurrentState.h<=fbWidth)
				offsetx = (mCurrentState.y + (int)(mCurrentState.h) -(int)fbWidth)/2;
   		}else if( Orientation==2 ) 
   		{
 			texCoords[2].u = 2;
    		texCoords[3].u = 2;  
    		if(mCurrentState.x+mCurrentState.w<=fbWidth)
				offsetx =(mCurrentState.x-(int)fbWidth)/2;
   		}else if(Orientation==3)
   		{
			texCoords[0].v = 2;
			texCoords[3].v = 2; 
			if(mCurrentState.y+mCurrentState.h<=fbWidth)
				offsetx=(-mCurrentState.y-(int)(mCurrentState.h))/2;
  		}
		glTranslatef(offsetx,offsety,0);
	}
	//LOGI("-- x %i  y%i w %i h %i ", mCurrentState.x,mCurrentState.y,mCurrentState.w,mCurrentState.h);
	//LOGI("offsetx %i offsety %i ", offsetx,offsety);
	if(need2XScale_new)
	{
		offsetx=-mCurrentState.x/2.66f;
		//LOGI("x %i offsetx %i ", mCurrentState.x,offsetx);
		offsety=(mCurrentState.y+mCurrentState.h)/4;
		//LOGI("y %i h %i offsety %i", mCurrentState.y,mCurrentState.h,offsety);
		glTranslatef(offsetx,offsety,0);
	}
    ...
    while (it != end) {
        ...
        const GLint sy = fbHeight - (r.top + r.height());
		if(need2XScale)
        	glScissor(r.left/2, sy, r.width(), r.height());
		else if(need2XScale_new)
		{
			glScissor(r.left*3/4+offsetx, sy*3/4+offsety, r.width(), r.height());
		}
      	else
        	glScissor(r.left, sy, r.width(), r.height());
        glDrawArrays(GL_TRIANGLE_FAN, 0, 4);
