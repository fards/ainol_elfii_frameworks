...
//                    Last2XScaleValue,request,request_width,request_height);
	...
	const DisplayHardware& hw(graphicPlane(0).displayHardware());
    HWComposer& hwc(hw.getHwComposer());
	if((request == 8)||((Last2XScaleValue==8)&&(request==0)))
	{
	...
	else if((request == 16)||((Last2XScaleValue==16)&&(request==0)))
	{
		...
		float scale_x=0.0f;
		float scale_y=0.0f;
		int tmp= request_width*1000/hw.getWidth();
		scale_x=(float)tmp/1000;
		tmp=request_height*1000/hw.getHeight();
	...
	else
	{
		...
		if (ret < 0) {
			...
			sscanf(videoholebuf, "%i %i %i %i %i", &holex,&holey,&holew,&holeh,&holelayer);
