...
void Layer::mixTextureData(uint8_t format ) const
{
	if(hasmixed)
		return;
	...
	if (buffer != 0) {
		...
		void* vaddr = NULL;
		buffer->lock(usage,&vaddr);
		mixdata(vaddr,nativebuffer,format);
		hasmixed=true;
	}
}
void Layer::mixdata( void* data,ANativeWindowBuffer* nativebuffer,int mixtype)const
{
	int stride = nativebuffer->stride;
	int width = nativebuffer->width;
	int height = nativebuffer->height;
	int pixformat = nativebuffer->format;
	int bpp=4;
	/*
	...
	*/
	if(pixformat==WINDOW_FORMAT_RGB_565 )
	{
		bpp=2;
	}
	
	LOGI("data: 0x%x, w:%i h:%i stride:%i,mixtype:%i,bpp%i",data,width,height,stride ,mixtype,bpp);
	/*
	char value[16];
	if(property_get("rw.surface.3d",value,"true")<2)
	{
		memset(data,0,width*height*bpp/2);
		return;
	}*/
	...
	if(mixtype==1)
	{
		...
		void* current=data;
		int halfline=width*bpp/2;
		LOGI("halfline %i",halfline);

		for(int i=0;i<height;i++)
		{
			memcpy(tmp,current,stride*bpp);
			for(int j=0;j<width;j++)
			{
				int k=j/2;
				if(j%2==0)
				{
					memcpy(current+j*bpp,tmp+k*bpp,bpp);
				}
				else
				{
					memcpy(current+j*bpp,tmp+halfline+k*bpp,bpp);
				}
	...
	{
		LOGI("to be supported format: STEREOSCOPIC_3D_FORMAT_TOP_BOTTOM");
	}else
	{
		LOGE("unknow format: %i ",mixtype);
	}
