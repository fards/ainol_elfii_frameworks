...
public class UsbStorageActivity extends Activity
    ...
    private void handleUsbStateChanged(Intent intent) {
        ...
        if (!connected) {
			if(mCPUModeChange){
				Log.i(TAG, "restore cpu mode");
				writeSysfs(CPU_MODE_PATH, mCPUMode);
	        	writeSysfs(CPU_MAX_FREQ_PATH, mCPUMaxFreq);
				mCPUModeChange = false;
			}
    ...
    public void onClick(View v) {
        if (v == mMountButton) {
			String CpuMode = SystemProperties.get("ro.usbstorage.cpumode");
			String MaxFreq = SystemProperties.get("ro.usbstorage.maxfreq");

			if(!("".equals(CpuMode)) && !("".equals(MaxFreq))){
				mCPUMode = readSysfs(CPU_MODE_PATH, "conservative");
	            mCPUMaxFreq = readSysfs(CPU_MAX_FREQ_PATH, "1200000");

				Log.i(TAG, "now:(CPU mode:" + mCPUMode + ", CPU max freq:" + mCPUMaxFreq + ")" +
					", prop set:(CPU mode:" + CpuMode + ", CPU max freq:" + MaxFreq + ")");

				writeSysfs(CPU_MODE_PATH, CpuMode);
	            writeSysfs(CPU_MAX_FREQ_PATH, MaxFreq);
	...
	//in order to constant cpu freq
	private boolean mCPUModeChange = false;
	private String mCPUMaxFreq = "1200000";
	private String mCPUMode = "performance";
    ...
    private static int writeSysfs(String path, String val) {
	 ...
	 private static String readSysfs(String path, String defVal) {
        ...
        try {
            ...
            Log.e(TAG, "IO Exception when read: " + path, e);
            return defVal;
        }    
    }
}
