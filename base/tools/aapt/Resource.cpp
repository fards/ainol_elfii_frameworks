...
{
    String8 manifest(bundle->getAndroidManifestFile());

    // First, look for a package file to parse.  This is required to
    // be able to generate the resource information.
    sp<AaptGroup> androidManifestFile =
            assets->getFiles().valueFor(manifest.getPathLeaf());
    if (androidManifestFile == NULL) {
        fprintf(stderr, "ERROR: No %s file found.\n", manifest.getPathLeaf().string());
        return UNKNOWN_ERROR;
    }
...
status_t
writeProguardForAndroidManifest(Bundle* bundle, ProguardKeepSet* keep, const sp<AaptAssets>& assets)
{
    status_t err;
    ResXMLTree tree;
    size_t len;
    ...
    // First, look for a package file to parse.  This is required to
    // be able to generate the resource information.
    assGroup = assets->getFiles().valueFor(manifest.getPathLeaf());
    if (assGroup == NULL) {
        fprintf(stderr, "ERROR: No %s file found.\n", manifest.getPathLeaf().string());
        return -1;
    }
...
writeProguardFile(Bundle* bundle, const sp<AaptAssets>& assets)
{
    ...
    err = writeProguardForAndroidManifest(bundle, &keep, assets);
    if (err < 0) {
        return err;
    }
