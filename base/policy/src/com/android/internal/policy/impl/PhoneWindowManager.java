...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    public void setInitialDisplaySize(int width, int height) {
        ...
        if(SystemProperties.getBoolean("ro.platform.has.mbxuimode",false))
            mStatusBarCanHide = shortSizeDp < 403;
		else if(SystemProperties.getBoolean("ro.platform.UI.800x480",false))
			mStatusBarCanHide = shortSizeDp < 400;
        else
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (false) {
            Log.d(TAG, "interceptKeyBeforeDispatching interceptKeyTi keyCode=" + keyCode + " down=" + down + " repeatCount="
        ...
        if(SystemProperties.getBoolean("ro.tv.shortcutkey", false)) {
           ...
           if((SystemProperties.getBoolean("tv.globalsetup_show_status", false)&&(keyCode != KeyEvent.KEYCODE_TV_SHORTCUTKEY_TVINFO))
              ||(keyCode == KeyEvent.KEYCODE_TV_SHORTCUTKEY_SOURCE_LIST)||(keyCode == KeyEvent.KEYCODE_TV_SLEEP)
              ||(keyCode == KeyEvent.KEYCODE_VOLUME_UP)||(keyCode == KeyEvent.KEYCODE_VOLUME_DOWN)
              ||(keyCode == KeyEvent.KEYCODE_TV_SHORTCUTKEY_3DMODE)||(keyCode == KeyEvent.KEYCODE_TV_SHORTCUTKEY_DISPAYMODE)
              ||(keyCode == KeyEvent.KEYCODE_TV_SHORTCUTKEY_VIEWMODE)||(keyCode == KeyEvent.KEYCODE_TV_SHORTCUTKEY_VOICEMODE)
              ||(keyCode == KeyEvent.KEYCODE_MUTE)||(keyCode == KeyEvent.KEYCODE_EARLY_POWER)) {
                if(down) {
                    Intent intent1=new Intent(KEY_DOWN_HAPPEN);
                    intent1.putExtra("keyNum",keyCode);
                    mContext.sendBroadcast(intent1);  
         	  ...
         	  if((keyCode == KeyEvent.KEYCODE_BACK)&&(SystemProperties.getBoolean("tv.shortcutkey_show", false))){
         	  	if(down) {
                    Intent intent1=new Intent(KEY_DOWN_HAPPEN);
                    intent1.putExtra("keyNum",keyCode);
                    mContext.sendBroadcast(intent1);  
    ...
    void launchHomeFromHotKey() {
        if (mKeyguardMediator.isShowingAndNotHidden()) {
		    ...
		    else if(SystemProperties.getBoolean("ro.tv.shortcutkey", false))
		    {
	   		 Log.i(TAG, " ############## tv platform startDockOrHome   ");
                         sendCloseSystemWindows(SYSTEM_DIALOG_REASON_HOME_KEY);
                         startDockOrHome();
		   }
                }
    ...
    public int finishAnimationLw() {
        ...
        if (mStatusBar != null) {
            if(SystemProperties.getBoolean("sys.statusbar.forcehide",false)== true||SystemProperties.getBoolean("statusbar.hide.setting",false)==true){
