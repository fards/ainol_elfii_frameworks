...
public class WindowManagerImpl implements WindowManager {
    ...
    static class CompatModeWrapper implements WindowManager {
        ...
        public int getMouseCursorType() {
            return mWindowManager.getMouseCursorType();
        }

        @Override
        public int setMouseCursorType(int type) {
            return mWindowManager.setMouseCursorType(type);
        }

        @Override
        public void setMouseStatus(boolean enabled) {
            mWindowManager.setMouseStatus(enabled);
        }
    }
        ...
        if (HardwareRenderer.isAvailable()) {
            switch (level) {
                ...
                case ComponentCallbacks2.TRIM_MEMORY_MODERATE:
                    ...
                    // treat our platform as low and mediun end gfx device
                    if (true/*!ActivityManager.isHighEndGfx(getDefaultDisplay())*/) {
    ...
    public int setMouseCursorType(int type) {
        IWindowManager wm = IWindowManager.Stub.asInterface(ServiceManager.getService("window"));
        try {
            return wm.setMouseCursorType(type);
        } catch (RemoteException e) {
        }
        return -1;
    }
