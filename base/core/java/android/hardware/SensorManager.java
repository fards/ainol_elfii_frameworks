...
public class SensorManager
{
	...
	//when no accelerometer or gyro, use remote control simulate a sensor device
    ...
    public boolean registerListener(SensorEventListener listener, Sensor sensor, int rate,
        ...
        synchronized (sListeners) {
			...
			if( ((!hasAccelerometer) && (Sensor.TYPE_ACCELEROMETER == sensor.getType())) ||
	...
	*/	
        ...
        public void run() {
			...
			while(true){
				...
				if(!mRegisterListener){
					Log.e(TAG, "RC, application not registerListener!!!");
					continue;
				}

				if( EVENT_TYPE_SENSOR == inStream[0] ){
					byte type;
	...
	private boolean connect() {
        ...
        try {
        	...
        	Log.e(TAG, "RC, connect exception" + ex);
            disconnect();
            return false;
        }
        return true;
