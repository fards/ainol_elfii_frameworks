...
public abstract class AdapterViewAnimator extends AdapterView<Adapter>
	...
	/*set the property for StackView2*/
	boolean mStackIndex = false;
    ...
    Log.e(TAG,"-------transformViewForTransition"+fromIndex+" "+toIndex);
        if (fromIndex == -1) {
    ...
    public void setDisplayedChild(int whichChild) {
    	Log.e(TAG,"----setDisplayedChild(int)");
    ...
    private void setDisplayedChild(int whichChild, boolean animate) {
        if (mAdapter != null) {
            ...
            Log.e(TAG,"----setDisplayedChild "+mWhichChild+hasFocus);
    ...
    public void showNext() {
    	Log.e(TAG,"----showNext(),mWhichChild : "+mWhichChild);
    ...
    Log.e(TAG,"************getViewAtRelativeIndex()"+relativeIndex+" "+mCurrentWindowStartUnbounded+" ");
    ...
    void refreshChildren() {
		...
		Log.e(TAG,"------refreshChildren mCurrentWindowStart"+mCurrentWindowStart+"mCurrentWindowEnd"+mCurrentWindowEnd);
        for (int i = mCurrentWindowStart; i <= mCurrentWindowEnd; i++) {// -1,0,1,2,3,4
			//Log.e(TAG,"------refreshChildren :"+i);	
	...
	void showOnly(int childIndex, boolean animate)
		{
        if (mAdapter == null) return;
		//animation is true,  mPreviousViews.size() is 0 at the first time;childIndex
		//childIndex --- 3
		Log.e(TAG,"-------showOnly()"+childIndex);
        ...
        for (Integer index : mViewsMap.keySet()) {
            ...
            if (remove) {
				...
				Log.e(TAG,"-------showOnly.clear"+oldRelativeIndex);
        ...
        if (!(newWindowStart == mCurrentWindowStart && newWindowEnd == mCurrentWindowEnd &&
            ...
            Log.e(TAG,"-------newWindowStart"+newWindowStart+" newWindowEnd"+newWindowEnd+" newWindowStartUnbounded"+newWindowStartUnbounded);
            for (int j = newWindowStart; j <= newWindowEnd; j++) { // -2,-1,0 ,1,2,3
            	int i=0;
            	if(mStackIndex)
					{
					int middle = newWindowStart+(newWindowEnd-newWindowStart+1)/2;
					if(j<=middle)
						{
						i = newWindowStart+(middle-j);
						}
					else
						i=j;
					}
                ...
                if (inOldRange) {
					...
					Log.e(TAG,"-------showOnly.transform: "+oldRelativeIndex);
                ...
                // Otherwise this view is new to the window
                    ...
                    final int adapterPosition = modulo(i, adapterCount);  //4,0,1,2,3,4
                    View newView = mAdapter.getView(adapterPosition, null, this);
                    long itemId = mAdapter.getItemId(adapterPosition); //0
					...
					Log.e(TAG,"-------index :"+index+"newRelativeIndex:"+newRelativeIndex+"adapterPosition:"+adapterPosition);
                    mViewsMap.put(index, new ViewAndMetaData(fl, newRelativeIndex,adapterPosition, itemId));
                    addChild(fl);
                    applyTransformForChildAtIndex(fl, newRelativeIndex);
					Log.e(TAG,"-------showOnly from -1 :"+newRelativeIndex);
    ...
    void checkForAndHandleDataChanged() {
		...
		Log.e(TAG,"----checkForAndHandleDataChanged:"+dataChanged);
        if (dataChanged) {
            post(new Runnable() {
                public void run() {
                    handleDataChanged();
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
		...
		Log.e(TAG,"*********childCount is"+childCount);
        for (int i = 0; i < childCount; i++) {
            final View child = getChildAt(i);
            int childRight = mPaddingLeft + child.getMeasuredWidth();
            int childBottom = mPaddingTop + child.getMeasuredHeight();
			Log.e(TAG,"*********childBottom is"+childBottom);
    ...
    public void onRestoreInstanceState(Parcelable state) {
        ...
        if (mRemoteViewsAdapter != null && mAdapter == null) {
        	...
        	Log.e(TAG,"----onRestoreInstanceState()"+mWhichChild);
    ...
    public boolean onRemoteAdapterConnected() {
        if (mRemoteViewsAdapter != mAdapter) {
            ...
            if (mRestoreWhichChild > -1) {
				Log.e(TAG,"----onRemoteAdapterConnected()");
