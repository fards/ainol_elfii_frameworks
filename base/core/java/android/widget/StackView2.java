 ...
 */
public class StackView2 extends AdapterViewAnimator {
    private final String TAG = "StackView2";
    ...
    private static final int STACK_RELAYOUT_DURATION = 600;
    ...
    private static final float PERSPECTIVE_SCALE_FACTOR = 0.8f;//0f;
	...
	private StackSlider mStackSlider1;
	private StackSlider mStackSlider2;
	private StackSlider mStackSlider3;
	private StackSlider mStackSlider4;
	private StackSlider mStackSlider5;
    /**
     * {@inheritDoc}
     */
    public StackView2(Context context) {
        this(context, null);
    }
     ...
     * {@inheritDoc}
     */
    public StackView2(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initStackView() {
		...
		mStackSlider1= new StackSlider();
		mStackSlider2= new StackSlider();
		mStackSlider3= new StackSlider();
		mStackSlider4= new StackSlider();
		mStackSlider5= new StackSlider();
		...
		setStackView2index(true);
		//Log.e(TAG,"#####setStackView2index###");
    }
	...
	void transformViewForTransition(int fromIndex, int toIndex, final View view, boolean animate) {
		//Log.e(TAG,"--------fromIndex:"+fromIndex+" toIndex: "+toIndex+animate);
        if (!animate) {
            ((StackFrame) view).cancelSliderAnimator();
            view.setRotationX(0f);
            LayoutParams lp = (LayoutParams) view.getLayoutParams();
    ...
    private void transformViewAtIndex(int index, final View view, boolean animate) {
        ...
        float r = (index * 1.0f) / (mMaxNumActiveViews - 2);  // 0,1/4,2/4,3/4,4/4;

		int itemPos = index > 2 ? 1 : 0;  // if index is from 0 to 2,it will be shown in upper part

        //final float scale = 1 - PERSPECTIVE_SCALE_FACTOR * (1 - r);
        //float perspectiveTranslationY = r * maxPerspectiveShiftY;
		final float scale = 1 - PERSPECTIVE_SCALE_FACTOR*(Math.abs(0.5f-r));
		//Log.e(TAG,"---index:"+index+"scale:"+scale+" maxPerspectiveShiftY:"+maxPerspectiveShiftY);
		
		float perspectiveTranslationY = 0.3f*maxPerspectiveShiftY*itemPos+ 0.7f*r* maxPerspectiveShiftY;
        float scaleShiftCorrectionY = (scale - 1) *
                (getMeasuredHeight() * (1 - PERSPECTIVE_SHIFT_FACTOR_Y) / 2.0f);
		//Log.e(TAG,"---scaleShiftCorrectionY:"+scaleShiftCorrectionY+" perspectiveTranslationY:"+perspectiveTranslationY);
		int sclCrctY = index > 2 ? -1 : 1;  
        final float transY = perspectiveTranslationY + sclCrctY*(scaleShiftCorrectionY+15);
	...
	private void setupStackSlider2(View v, int mode , boolean isFocus ,StackSlider mStkSlider) {
        mStkSlider.setMode(mode);
        if (v != null) {
			/*if(isFocus)
				{
					mHighlight.setImageBitmap(sHolographicHelper.createResOutline(v, mResOutColor));
            ...
            v.bringToFront();
            mStkSlider.setView(v);
            v.setVisibility(VISIBLE);
        }
    }
	
	@Override
	public void setStackView2index(boolean bl)
		{
		super.setStackView2index(bl);
		}
    ...
    public void showNext() {
    	//Log.e(TAG,"---------showNext():"+mSwipeGestureType+mTransitionIsSetup);
    ...
    public void showPrevious() {
    	//Log.e(TAG,"---------showPrevious() :"+mSwipeGestureType+mTransitionIsSetup);
        if (mSwipeGestureType != GESTURE_NONE) return;
        if (!mTransitionIsSetup) {
    ...
    void showOnly(int childIndex, boolean animate) {
        ...
        //Log.e(TAG,"------showOnly()--mCurrentWindowEnd:"+mCurrentWindowEnd+"mCurrentWindowStart:"+mCurrentWindowStart);
        for (int j = mCurrentWindowEnd; j >= mCurrentWindowStart; j--) { //3,2,1,0,-1,-2--->3,2,-2,-1,0,1
			
				int middle = mCurrentWindowStart+(mCurrentWindowEnd-mCurrentWindowStart+1)/2;  //1
				int i=0;
				if(j<=middle)
					{
					i = mCurrentWindowStart+(middle-j);   //
					}
				else
					i=j;	
            int index = modulo(i, getWindowSize());
			//Log.e(TAG,"------showOnly()--index:"+index+"i : "+i);
    ...
    private void updateChildTransforms() {
		//Log.e(TAG,"-----updateChildTransforms() "+getNumActiveViews());
    ...
    /*private void beginGestureIfNeeded(float deltaY) {
        if ((int) Math.abs(deltaY) > mTouchSlop && mSwipeGestureType == GESTURE_NONE) {
			...
			Log.e(TAG,"-------------activeIndex:"+activeIndex);
	...
	private void beginGestureIfNeeded(float deltaY) {
        if ((int) Math.abs(deltaY) > mTouchSlop && mSwipeGestureType == GESTURE_NONE) {
           ...
           // View v = getViewAtRelativeIndex(activeIndex);
			...
			View v1 = getViewAtRelativeIndex(1);
			View v2 = getViewAtRelativeIndex(2);
			View v3 = getViewAtRelativeIndex(3);
			View v4 = getViewAtRelativeIndex(4);
			View v5 = getViewAtRelativeIndex(5);
            //if (v == null) return;
			if ((v1 == null)||(v2 == null)||(v3 == null)||(v4 == null)||(v5 == null)) return;
            //setupStackSlider(v, stackMode);
			
			setupStackSlider2(v1, stackMode ,false ,mStackSlider1);
			setupStackSlider2(v2, stackMode ,false ,mStackSlider2);
			setupStackSlider2(v5, stackMode ,false ,mStackSlider5);
			setupStackSlider2(v4, stackMode ,false ,mStackSlider4);
			setupStackSlider2(v3, stackMode ,true ,mStackSlider3);
            // We only register this gesture if we've made it this far without a problem
            mSwipeGestureType = swipeGestureType;
            cancelHandleClick();
        }
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        mVelocityTracker.addMovement(ev);
		//Log.d(TAG, "------------------onTouchEvent: "+action+"deltaY:"+deltaY);
        switch (action & MotionEvent.ACTION_MASK) {
            case MotionEvent.ACTION_MOVE: {
                beginGestureIfNeeded(deltaY);

				mStackSlider5.setIndex(5);
                mStackSlider4.setIndex(4); 
                mStackSlider1.setIndex(1);
                mStackSlider2.setIndex(2);
                mStackSlider3.setIndex(3); 
					
				float rx = deltaX / (mSlideAmount * 1.0f);
                if (mSwipeGestureType == GESTURE_SLIDE_DOWN) {
                    //float r = (deltaY - mTouchSlop * 1.0f) / mSlideAmount * 1.0f;
					float r = (deltaY - mTouchSlop * 1.0f) /89.4f;
                    if (mStackMode == ITEMS_SLIDE_DOWN) r = 1 - r;
					//Log.d(TAG, "-----GESTURE_SLIDE_DOWN-r: "+r);
					/*mStackSlider.setYProgress(1 - r);
                    mStackSlider.setXProgress(rx);*/
					mStackSlider5.setYProgress(1 - r);
					if(r< 0.5f)
						mStackSlider4.setYProgress(1 - r);
                    mStackSlider1.setYProgress(1 - r);
					mStackSlider2.setYProgress(1 - r); 
					mStackSlider3.setYProgress(1 - r);   
					if(r>=0.5f)
						mStackSlider4.setYProgress(1 - r);
                    return true;
                } else if (mSwipeGestureType == GESTURE_SLIDE_UP) {
					...
					float r = (deltaY + mTouchSlop * 1.0f) /89.4f;
                    if (mStackMode == ITEMS_SLIDE_DOWN) r = 1 - r;
					//Log.d(TAG, "--GESTURE_SLIDE_UP---r: "+r);
                    /*mStackSlider.setYProgress(r);
                    mStackSlider.setXProgress(rx);*/
                    mStackSlider5.setYProgress(1-r);	
                    mStackSlider4.setYProgress(1-r);
    ...
    private void handlePointerUp(MotionEvent ev) {
        ...
        if (deltaY > mSwipeThreshold && mSwipeGestureType == GESTURE_SLIDE_DOWN
            ...
            CancelStackSlider(mStackSlider5,5);
			CancelStackSlider(mStackSlider4,4);
			CancelStackSlider(mStackSlider1,1);
			CancelStackSlider(mStackSlider2,2);
			CancelStackSlider(mStackSlider3,3);
	...
	private void CancelStackSlider(StackSlider mStkSlider, int index)	
		{
			mStkSlider.setYProgress(0.0f);
        }
    ...
    private class StackSlider {
        ...
        public void setYProgress(float r ) {
            ...
            int mScaleType = 1; //1: scale bigger  -1: scale smaller
            //Log.e(TAG,"-----------setYProgress(),"+r+" "+mMode+" mIndex:"+mIndex);
            if(mIndex<3)
				{
				mScaleType = -1;
				}
            ...
            switch (mMode) {
                case NORMAL_MODE:
					int isMiddle = (mIndex == 3) ? 1 :0;
					float scaleShiftCorrectionY = 0.2f*(getMeasuredHeight() * (1 - PERSPECTIVE_SHIFT_FACTOR_Y) / 2.0f);
					//Log.e(TAG,"-------scaleShiftCorrectionY :"+scaleShiftCorrectionY);
					//0.2 is PERSPECTIVE_SCALE_FACTOR / (mMaxNumActiveViews - 2)
					float mItemTransAmount = 0.65f*0.25f*maxPerspectiveShiftY+isMiddle*0.35f*maxPerspectiveShiftY;
					//Log.e(TAG,"-------mItemTransAmount :"+mItemTransAmount);
					float mItemSlideAmount = mItemTransAmount+scaleShiftCorrectionY;
					int mVerticalOffset= Math.round(-r * stackDirection * mItemSlideAmount);
					viewLp.setVerticalOffset(mVerticalOffset);
                    ...
                    float alpha = (isMiddle > 0 ) ? (float)(0.5f+Math.abs(r-0.5)) : 1.0f;

					float r1 = ((mIndex-1) * 1.0f) / (mMaxNumActiveViews - 2);  // 0,1/4,2/4,3/4,4/4;
					
					int sclCrctY = mIndex <=3 ? -1 : 1;  
					if((r<0)&&(mIndex == 3))
						sclCrctY = 1;
					float now_scale = 1 - PERSPECTIVE_SCALE_FACTOR*(Math.abs(0.5f-r1));
					float scale = now_scale+0.2f*r*sclCrctY;
					...
					//Log.e(TAG,"-------alpha :"+alpha+" scale:"+scale+" mItemSlideAmount:"+mItemSlideAmount);
					//Log.e(TAG,"-------setVerticalOffset :"+mVerticalOffset);
    ...
    protected void onLayout(boolean changed, int left, int top, int right, int bottom) {
        ...
        final int childCount = getChildCount();//childCount is 8
        for (int i = 0; i < childCount; i++) {		
            final View child = getChildAt(i);
    ...
    private void measureChildren() {
        ...
        for (int i = 0; i < count; i++) {
			...
			if (child != mClickFeedback){
                final int childMeasuredWidth = child.getMeasuredWidth();
                final int childMeasuredHeight = child.getMeasuredHeight();
				//Log.e(TAG,"------childMeasuredWidth"+childMeasuredWidth+" "+childMeasuredHeight);
				...
				//Log.e(TAG,"------maxWidth"+maxWidth+" "+maxHeight);
            }
        }
		...
		//Log.e(TAG,"------mNewPerspectiveShiftX"+mNewPerspectiveShiftX+" "+mNewPerspectiveShiftY);
