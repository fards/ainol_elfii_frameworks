...
public class ConnectivityManager {
     ...
     * {@hide}
     */
    public void setGlobalProxy(ProxyProperties prxy, boolean jumpOver) {
    	int index;
	String exclList="";
	String host= prxy.getHost();
	int port = prxy.getPort();
	if(jumpOver)
	{
	        try {
			for(LinkAddress linkaddr : getActiveLinkProperties().getLinkAddresses())
			{
				String ipaddr = linkaddr.getAddress().getHostAddress ();
				index = ipaddr.lastIndexOf ("."); 
				exclList = ipaddr.substring(0, index); 
				exclList = exclList + ".0/24,";
			}
			exclList = exclList.substring(0, exclList.length()-1);
			ProxyProperties tmp = new ProxyProperties(host, port, exclList);
	            mService.setGlobalProxy(tmp);
	        } catch (RemoteException e) {
	        }
	}
	else
