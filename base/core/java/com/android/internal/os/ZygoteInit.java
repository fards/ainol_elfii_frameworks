...
public class ZygoteInit {
    ...
    static void preload() {
	if("1".equals(SystemProperties.get("ro.amlogic.no.preloadclass"))){
		Slog.v(TAG, "Couldn't to load class! ro.amlogic.no.preloadclass:" + SystemProperties.get("ro.amlogic.no.preloadclass"));
	}else{
        	preloadClasses();
	}
	if("1".equals(SystemProperties.get("ro.amlogic.no.preloadresources"))){
		Slog.v(TAG, "Couldn't to load resource! ro.amlogic.no.preloadresources:" + SystemProperties.get("ro.amlogic.no.preloadresources"));
	}else{
        	preloadResources();
	}
    }
