                                                     ...
                                                     jobject clazz)
    {
        ...
        if((len = recvmsg(nl_socket_poll, &msg, 0))>= 0) {
                 ...
                 nh = NLMSG_NEXT (nh, len))
            {
                ...
                LOGV("event :%d  found flags=%X", nh->nlmsg_type, einfo->ifi_flags);
                if (nh->nlmsg_type == RTM_DELLINK ||
                    ...
                    if (type == RTM_NEWLINK &&
                        (!(einfo->ifi_flags & IFF_LOWER_UP))) {
                            if (total_int > last_total_cnt) {
                                LOGI("Had device add: last = %d; now = %d", last_total_cnt, total_int);
                            } else {
                                type = RTM_DELLINK;
                            }
                    }
                    if ((info = find_info_by_index
                        ...
                        if(total_int>0){
                            ...
                            snprintf(result,left, "(null):%d:",type);
                        }
                    }
            ...
            rbuf[4096 - left] = '\0';
            LOGD("poll state :%s, left:%d",rbuf, left);
        }
...
static int is_wire_inserted(const char *name)
{
    ...
    if (ret != 0) {
        LOGD( "%s(%s) failed with ret = %d\n", __FUNCTION__, name, ret);
        return 0;
    }

    LOGD( "%s: flags = 0x%x, IFF_LOWER_%s\n", name, flags,
