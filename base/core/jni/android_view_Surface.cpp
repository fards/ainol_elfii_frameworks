...
static void Surface_set3DFormat(
        JNIEnv* env, jobject clazz, jint format)
{
	LOGI("Surface_set3DFormat %i",format);
    const sp<SurfaceControl>& surface(getSurfaceControl(env, clazz));
    if (surface == 0) return;
    status_t err = surface->set3DFormat( format);
    if (err<0 && err!=NO_INIT) {
        doThrowIAE(env);
    }
}
...
static JNINativeMethod gSurfaceMethods[] = {
    ...
    {"set3DFormat",         "(I)V",(void*)Surface_set3DFormat },
