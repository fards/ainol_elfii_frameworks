...
MediaScanner::MediaScanner()
    ...
    loadSkipList();
       ...
       mHaveExternalStorage2=false;
       pExternalStorage=NULL;
       pExternalStorage2=NULL;

         char * p;
        if((p = getenv("EXTERNAL_STORAGE2"))){
            int len=strlen(p);
            pExternalStorage2 = (char *)malloc(len + 2);
            strcpy(pExternalStorage2, p);
             if (len > 0 && pExternalStorage2[len - 1] == '/') {
                pExternalStorage2[len-1] = 0;//have no '/' as end
            }
            LOGD("EXTERNAL_STORAGE2 = %s",pExternalStorage2);
	     if((p=getenv("EXTERNAL_STORAGE"))){
                 len=strlen(p);
                 pExternalStorage = (char *)malloc(len + 2);
                 strcpy(pExternalStorage, p);
                 if (len > 0 && pExternalStorage[len - 1] != '/') {
                    pExternalStorage[len] = '/';
                    pExternalStorage[len + 1] = 0;
                 }
		        mHaveExternalStorage2=true;
	     	}
        }
...
MediaScanner::~MediaScanner() {
    ...
    free(mSkipList);
    free(mSkipIndex);
    if(pExternalStorage2)
	free(pExternalStorage2);
    if(pExternalStorage)
	free(pExternalStorage);
}
...
MediaScanResult MediaScanner::processDirectory(
    ...
    mBypassExternalStorage2= (mHaveExternalStorage2 && (!strcmp(pathBuffer,pExternalStorage)));
...
MediaScanResult MediaScanner::doProcessDirectory(
    ...
    struct dirent* entry;
    //LOGD("doProcessDirectory %s",path);
    ...
    if(mBypassExternalStorage2) {
        //check if the externstorage2 in current directory
        int len = strlen(path);
        if(NULL == strchr(pExternalStorage2+len,'/')) {
            folderName = pExternalStorage2+len;
            LOGD("folderName %s",folderName);
            mBypassExternalStorage2 = false;//no need to check it again
        }
    }
...
MediaScanResult MediaScanner::doProcessDirectoryEntry(
   ...
   // LOGD("doProcessDirectoryEntry %s",name);
